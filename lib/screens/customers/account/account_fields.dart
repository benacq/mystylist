...
class _AccountFieldsState extends State<AccountFields> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        GetBuilder<CustomerController>(builder: (state) {
          return AccountData(
            label: 'Email',
            textFormField: Form(
              key: CustomerController.formKeys[0],
              child: TextFormField(
            ...
            handlerButton: GestureDetector(
              ...
              onTap: () {
                FormState formState =
                    CustomerController.formKeys[0].currentState;

                if (formState.validate()) {
                  formState.save();
                  if (!email.isNull && email != widget.model.email) {
                    AuthController().changeEmail(email).then(
                        (value) => state.updateSingleData(0, "email", email));
                  } else {
        ...
        GetBuilder<CustomerController>(builder: (state) {
          return AccountData(
            label: 'Phone number',
            textFormField: Form(
              key: CustomerController.formKeys[1],
              child: TextFormField(
            ...
            handlerButton: GestureDetector(
              onTap: () {
                FormState formState =
                    CustomerController.formKeys[1].currentState;
                if (formState.validate()) {
                  formState.save();
                  if (!contact.isNull && contact != widget.model.contact) {
                    state.updateSingleData(1, "contact", contact);
                  } else {
        ...
        GetBuilder<CustomerController>(builder: (state) {
          return AccountData(
            label: 'Location',
            textFormField: Form(
              key: CustomerController.formKeys[2],
              child: TextFormField(
            ...
            handlerButton: GestureDetector(
              onTap: () {
                FormState formState =
                    CustomerController.formKeys[2].currentState;
                if (formState.validate()) {
                  formState.save();
                  if (!location.isNull && location != widget.model.location) {
                    state.updateSingleData(2, "location", location);
                  } else {
        ...
        GetBuilder<CustomerController>(builder: (state) {
          return AccountData(
            label: 'Account Type',
            textFormField: Form(
              key: CustomerController.formKeys[3],
              child: TextFormField(
                  initialValue: widget.model.accountType,
                  style: GoogleFonts.lato(
                    textStyle: TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.w600,
